        ...
        switch (evento.getTipo()) {
            case Evento.EVENTO_CAMBIO:
                if(evento.getPartida().getTablero().getEstado()!=Tablero.EN_CURSO) {
                    InterfazConServidor.getServer(evento.getPartida().getActivity()).newmovement(C3Preference.getPlayerId(evento.getPartida().getActivity()), C3Preference.getPartidaId(evento.getPartida().getActivity()), evento.getPartida().getTablero().tableroToString()+"&finished", listenerJSON, errorListenerJSON);
